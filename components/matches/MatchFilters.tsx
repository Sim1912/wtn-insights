...
function FilterPicker<T extends string>({ label, value, options, onChange }: { label: string; value: T; options: PickerOption<T>[]; onChange: (value: T) => void }) {
  ...
  return <div className="filter-picker" ref={picker}>
    <button type="button" className="filter-picker-trigger" aria-expanded={open} aria-controls={menuId} aria-label={`${label}: ${selectedLabel}`} onClick={() => setOpen((current) => !current)}><span>{selectedLabel}</span><b aria-hidden="true">⌄</b></button>
    {open && <div id={menuId} className="filter-picker-menu" role="group" aria-label={label}>{options.map((option) => <button key={option.value} type="button" aria-pressed={option.value === value} className={option.value === value ? "active" : ""} onClick={() => { onChange(option.value); setOpen(false); }}><span>{option.label}</span>{option.value === value && <b aria-hidden="true">✓</b>}</button>)}</div>}
  </div>;
}
