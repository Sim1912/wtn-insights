...
type TrendMetric = "winRate" | "setsWonRate" | "gamesWonRate";
...
function TrendTooltip({ active, payload, metric }: { active?: boolean; payload?: Array<{ payload?: TrendPoint }>; metric: TrendMetric }) {
  ...
  const labels: Record<TrendMetric, string> = { winRate: "Win rate", setsWonRate: "Sets won", gamesWonRate: "Games won" };
  const numerator = metric === "winRate" ? point.wins : metric === "setsWonRate" ? point.setsWon : point.gamesWon;
  const denominator = metric === "winRate" ? point.matches : metric === "setsWonRate" ? point.setsWon + point.setsLost : point.gamesWon + point.gamesLost;
  return <div className="rating-tooltip analytics-tooltip" role="status"><strong>{monthFormatter.format(new Date(`${point.month}-01T00:00:00Z`))}</strong><div className="tooltip-primary"><span>{labels[metric]}</span><strong>{pct(point[metric])}</strong></div><div className="tooltip-row"><span>Record</span><b>{numerator} of {denominator}</b></div><div className="tooltip-row"><span>Matches</span><b>{point.matches}</b></div></div>;
}
...
export function AnalyticsPage() {
  ...
  const trendLabel: Record<TrendMetric, string> = { winRate: "Win rate", setsWonRate: "Sets won", gamesWonRate: "Games won" };
  const trendColor: Record<TrendMetric, string> = { winRate: "#b7dc22", setsWonRate: "#6e91d9", gamesWonRate: "#ca765e" };
    ...
    {!data && status === "error" ? <section className="load-error shell"><strong>Analytics could not be loaded.</strong><p>{message}</p><button type="button" onClick={() => void load(playerId)}>Try again</button></section> : !data ? <div className="analytics-loading shell"><i /><i /><i /></div> : <div className="analytics-shell shell">
      ...
      <section className="analytics-section" aria-labelledby="opponents-title">
        ...
        <div className="analytics-charts">
          <article className="trend-panel"><header><div><h3>Form over time</h3><p>Discrete monthly results</p></div><select aria-label="Trend metric" value={trendMetric} onChange={(event) => setTrendMetric(event.target.value as TrendMetric)}><option value="winRate">Win rate</option><option value="setsWonRate">Sets won</option><option value="gamesWonRate">Games won</option></select></header><div className="analytics-chart-frame">{report.trends.length > 1 ? <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={220}><LineChart data={report.trends} accessibilityLayer margin={{ top: 16, right: 14, bottom: 4, left: 0 }}><CartesianGrid vertical={false} stroke="#e2e6dd" strokeDasharray="3 7" /><XAxis dataKey="month" tickFormatter={(value) => monthFormatter.format(new Date(`${value}-01T00:00:00Z`))} tickLine={false} axisLine={false} minTickGap={35} tick={{ fill: "#626b5e", fontSize: 10 }} /><YAxis domain={[0, 1]} ticks={[0, .25, .5, .75, 1]} tickFormatter={(value) => `${value * 100}%`} width={42} tickLine={false} axisLine={false} tick={{ fill: "#626b5e", fontSize: 10 }} /><Tooltip content={(props) => <TrendTooltip active={props.active} payload={props.payload as Array<{ payload?: TrendPoint }>} metric={trendMetric} />} isAnimationActive="auto" /><Line dataKey={trendMetric} name={trendLabel[trendMetric]} type="linear" connectNulls stroke={trendColor[trendMetric]} strokeWidth={3} dot={{ r: 4, strokeWidth: 2, fill: "#fff" }} activeDot={{ r: 6 }} isAnimationActive="auto" animationDuration={180} /></LineChart></ResponsiveContainer> : <p className="analytics-empty">Not enough dated months.</p>}</div></article>
